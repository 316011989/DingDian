package cn.yumi.daka.data.remote
...
class RemoteDataSource : IDataSource {
    ...
    override fun wechatLogin(params: WeakHashMap<String, String>): MutableLiveData<LoginResponse?> {
        val result = MutableLiveData<LoginResponse?>()
        ApiManager.instance.getApi().wechatLogin(params).enqueue(object : Callback<String> {
            override fun onFailure(call: Call<String>, t: Throwable) {
                result.value = null
            }
            ...
            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (!TextUtils.isEmpty(response.body())) {
                    ALogger.json("wechatLogin", response.body())
                    try {
                        result.value = Gson().fromJson(
                            response.body(),
                            LoginResponse::class.java
                        )
                    } catch (e: Exception) {
        ...
        ApiManager.instance.getApi().videoSuggest(id).enqueue(object : Callback<String> {
            ...
            override fun onResponse(call: Call<String>, response: Response<String>) {
                if (!TextUtils.isEmpty(response.body())) {
                    ...
                    try {
                        val videoData =
                            Gson().fromJson(response.body(), VideoSuggest::class.java)
                        result.value = videoData
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
