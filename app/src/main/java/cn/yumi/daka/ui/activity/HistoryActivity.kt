...
import  cn.yumi.daka.data.remote.model.HistoryWatchType
import cn.yumi.daka.data.local.db.entity.MovieHistoryEntity
import cn.yumi.daka.ui.adapter.HistoryTypeAdapter
...
class HistoryActivity : BaseActivity() {
    ...
    private fun getHistory(historyList: MutableList<MovieHistoryEntity>) {
        try {
            val dateList = mutableSetOf<String>()
            if (historyList.size > 0) {
                val history = mutableListOf<HistoryWatchType>()
                ...
                dateList.forEach { date ->
                    val hisDate = HistoryWatchType(HistoryWatchType.DATE)
                    if (date == today) {
                        hisDate.date = "今天"
                    } else {
                        hisDate.date = date
                    }
                    val hisList = HistoryWatchType(HistoryWatchType.LIST)
