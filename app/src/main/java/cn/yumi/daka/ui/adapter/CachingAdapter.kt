...
import cn.yumi.daka.data.remote.model.Detail
...
import cn.yumi.daka.download.SingleDownloadRunnable
import cn.yumi.daka.ui.activity.SettingActivity
    ...
    override fun convert(helper: BaseViewHolder?, item: DownloadEpisodeEntity?) {
        ...
        if (item.downloadStatus != null && item.downloadPrograss != null) {
            ...
            helper.getView<TextView>(R.id.item_download_downloadinfo).text =
                "$state  进度${progress / 100F}%"
            helper.getView<ProgressBar>(R.id.item_download_prograss).progress = progress
