...
public class PlayGestureCover extends BaseCover implements OnTouchGestureListener {
    ...
    public PlayGestureCover(Context context) {
        super(context);
    ...
    private void onHorizontalSlide(float percent) {
        ...
        if (showDelta != 0) {
            ...
            //判断快进、快退
            if (showDelta > 0) {
                forwardImg.setImageResource(R.mipmap.play_cover_forward_icon);
            } else {
                forwardImg.setImageResource(R.mipmap.play_cover_rewind_icon);
            }
