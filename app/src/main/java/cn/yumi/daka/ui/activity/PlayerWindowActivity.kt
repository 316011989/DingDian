...
import cn.yumi.daka.base.DataInter.Key.*
import cn.yumi.daka.base.DataInter.ReceiverKey.*
...
import cn.yumi.daka.viewmodel.PlayerViewModel
...
class PlayerWindowActivity : BaseActivity(), OnPlayerEventListener, OnErrorEventListener {
    ...
    private var interactionShowing: Boolean = false//插屏广告显示状态
    ...
    private fun p2pSpeed(playUrl: String?): Boolean {
        if (dataSource?.from == PlayerHelper.SOURCE_CC) {
            val domain = URL(playUrl!!).host
            val host: String
            if (dataSource?.extra!!.isNotEmpty()) {
                host = dataSource?.extra!!["Host"]!!.trim()
                if (!TextUtils.isEmpty(host) && host.startsWith("p2p")) {
                    p2pOriginUrl = playUrl.replace(domain, host)
                    val playU =
                        P2PManager.getInstance().loadVodUrl(p2pOriginUrl, domain, null, 0, 50)
                    P2PManager.getInstance().setCurrentVodUrl(p2pOriginUrl)
                    sendUrlToPlayer(playU)
                    return true
                }
            } else if (domain.startsWith("p2p")) {
                p2pOriginUrl = playUrl
                val playU =
                    P2PManager.getInstance().loadVodUrl(p2pOriginUrl, null, null, 0, 50)
                P2PManager.getInstance().setCurrentVodUrl(p2pOriginUrl)
                sendUrlToPlayer(playU)
                return true
            }
            ...
            if (video != null)
                runOnUiThread {
                    ...
                    if (localFile != null && localFile.exists()) {
                        ...
                        dataSource?.data = App.INSTANCE.createWifiUrl(
                            LeCast.getInstance().wifiIp,
                            localFile.path
                        )
    ...
    private fun updateVideoLayout() {
        ...
        if (mAssist != null) {
            val ratioSource =
                mAssist!!.mVideoWidth.toDouble() / mAssist!!.mVideoHeight.toDouble()
    ...
    private fun setSourcesData(sourcesData: List<VideoSources>?) {
        if (sourcesData != null && sourcesData.isNotEmpty()) {
            ...
            playerHelper?.addCacheRate()//playerHelper更新cover的可选清晰度
    ...
    private fun setVideoSuggest(suggests: MutableList<Suggest>) {
        if (suggests.size > 0) {
            ...
            playerUI?.initRecommendListView(suggests) { position ->
                ...
                TCAgentUtil.videoClick("${suggests[position].id}", "", "otherP")
            }
        }
    }
    ...
    private fun setPlayUrl() {
        ...
        //释放p2p资源
        if (!TextUtils.isEmpty(p2pOriginUrl)) {
            P2PManager.getInstance().unloadVodUrl(p2pOriginUrl)
            p2pOriginUrl = ""
        }

        playerHelper?.setPlayUrl(videoPlay!!, {
            if (ConfigCenter.parseUrl4Station != null
                && ConfigCenter.parseUrl4Station!!.source.contains(it.source)
            ) {
                //四大站解析的token有效期只有10秒,延迟5秒进行解析
//                handler.postDelayed({ parseAYTM(it.playUrl) }, 5000)
     ...
     * @param playUrl 原地址
     */
    private fun parseAYTM(playUrl: String) {
        model?.parseAYTM(ConfigCenter.parseUrl4Station!!.parseUrl + playUrl)?.observe(this) {
    ...
    fun changeResource() {
        ...
        when {
            ...
            videoData?.plays!!.size <= PlayerHelper.CHOOSEN_EPISODE_INDEX -> {
                PlayerHelper.CHOOSEN_EPISODE_INDEX = 0
    ...
    fun changeEsp(position: Int) {//18920349872
        ...
        playStart = 0
        setPlayUrl()
        TCAgentUtil.videoClick("${videoPlay!!.id}", "", "otherP")
    }
    ...
    override fun onResume() {
        ...
        if (mAssist?.isInPlaybackState != null
            && mAssist?.isInPlaybackState!!
            && PlayerHelper.CAST_SCREEN != 1
        ) {
            mAssist?.resume()
        }
    }
    ...
    private fun pauseVideo() {
        if (mAssist == null || mAssist!!.state == IPlayer.STATE_PLAYBACK_COMPLETE)
            return
    ...
    override fun onDestroy() {
        try {
            ...
            releasePlayer()
            //释放p2p资源
            if (!TextUtils.isEmpty(p2pOriginUrl)) P2PManager.getInstance()
                .unloadVodUrl(p2pOriginUrl)
    ...
    override fun onErrorEvent(eventCode: Int, bundle: Bundle?) {
        ...
        when (eventCode) {
            OnErrorEventListener.ERROR_EVENT_UNKNOWN -> {
                ...
                errorStr = "ERROR_EVENT_UNKNOWN" + bundle?.getString("errorMessage")
            }
        ...
        submitReportToMain("视频播放失败=${errorStr}", Api.PLAYER_ERROR_CODE)
        if (videoData != null)
            TCAgentUtil.videoPlayFail(dataSource?.data!!, videoData!!.source.toString())
    }
    ...
    private fun requestAllAd() {
        ...
        if (ConfigCenter.adControl?.fullscreenvideoTimes!! > 0
            ...
            ConfigCenter.adControl?.fullscreenvideoTimes =
                ConfigCenter.adControl?.fullscreenvideoTimes!! - 1
            App.INSTANCE.adFullscreenVideoTool!!.showFullVideoAd(this)
            mAssist?.pause()
            interactionShowing = true

            Log.d("PlayerWindowActivity", "全屏视频广告")
            App.INSTANCE.adFullscreenVideoTool!!.setDismiss {
                if (ConfigCenter.parseUrl4Station != null
                    && ConfigCenter.parseUrl4Station!!.source.contains(videoPlay!!.source)
                ) {
                    parseAYTM(videoPlay!!.playUrl)
                } else if (interactionShowing) {
        ...
        } else if (ConfigCenter.parseUrl4Station != null && ConfigCenter.parseUrl4Station!!.source.contains(
                videoPlay!!.source
            )
        ) {
            parseAYTM(videoPlay!!.playUrl)
     ...
     * 错误上报 我方服务器
     */
    private fun submitReportToMain(value: String, code: String) {
        if (videoData != null && videoPlay != null && dataSource != null && mAssist != null) {
            val reportJson = JsonObject()
            if (!TextUtils.isEmpty(value)) {
                reportJson.addProperty("failDetails", value)
                reportJson.addProperty("failType", code)
            }
            reportJson.addProperty("appId", 0)
            reportJson.addProperty("videoId", videoData!!.id)
            reportJson.addProperty("videoPlayId", videoPlay!!.id)
            reportJson.addProperty("name", videoData?.name)
            reportJson.addProperty("type", videoData?.type)
            reportJson.addProperty("source", dataSource?.from)
            if (videoPlay!!.playUrl != null && videoPlay!!.playUrl != "")
                reportJson.addProperty("ip", URL(videoPlay!!.playUrl).host)
            if (dataSource!!.extra != null) {
                reportJson.addProperty("host", dataSource!!.extra["host"])
            }
            reportJson.addProperty("url", dataSource?.data)
            reportJson.addProperty(
                "errTime",
                CommonUtil.timeStamp2DateStr(mAssist?.currentPosition!!.toLong(), "mm:ss")
            )
            reportJson.addProperty(
                "stayTime",
                CommonUtil.timeStamp2DateStr(System.currentTimeMillis() - enterTime, "mm:ss")
            )
            reportJson.addProperty("errorType", code)
            reportJson.addProperty("parseMessage", value)
            reportJson.addProperty("playMessage", value)
            DataRepository.instance.errorReport(reportJson.toString())
        }
